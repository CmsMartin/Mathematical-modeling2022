...
\textbf{对于问题一}：针对两种目标，我们先通过枚举核酸检测点所在的边，将原问题转化为单变量优化问题，再经过分析，得出结论：使总路程最小时，核酸检测点一定在点上；使最大路程最小时，核酸检测点在图的绝对中心上。最后，我们提出了复杂度可以接受的两种算法

\textbf{对于问题二}：我们把问题一的结论进行推广，并针对两种目标提出相应的算法。

\textbf{对于问题三}：针对目标一，我们先建立了整数规划模型，再提出了一种基于模拟退火的智能算法来求出近似最优解。
针对目标二，我们先用二分法把原问题转化为 Set cover problem(SCP)，再针对该问题建立了整数线性规划模型，由于 Set cover porblem 是 NP-Hard 的，我们使用了一种 Greedy 算法来近似地求解该问题。
